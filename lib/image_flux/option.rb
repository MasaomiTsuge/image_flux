...
class ImageFlux::Option
  ...
  ALLOWED_ROTATE = %w[1 2 3 4 5 6 7 8 auto]
  attribute :r, :string, default: "1", aliases: %i[rotate] do
    validate do |value|
      "rotate should be inclusion of #{ALLOWED_ROTATE.join(', ')}" unless ALLOWED_ROTATE.include?(value.to_s)
    end
  end
  # overlay attributes
  ...
  attribute :through, :string, default: nil
